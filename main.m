...
for frameIndex = 1:frame_total 
    ...
    elseif strcmp(filterSelection, 'MCF') == 1
        ...
        % ------------------------------------------------------------%
        % Wang et al. (2019) motion-consistency optical flow filter.
        % Fits a local plane to the 3×3 spatiotemporal neighbourhood
        % of each event and validates the resulting flow velocity.
        % Events with |v| = 0 (static) or |v| > V_max (noise-induced
        % outlier) are rejected.
    ...
    elseif strcmp(filterSelection, 'COH') == 1
        ...
        % % Compute exponential moving average for th_lo
        % threshold_smoothing = 0.1;
        % 
        % if frameIndex == 1
        %     th_low_ema = th_lo;
        % else
        %     prev_ema = coh_logs.filter_threshold(frameIndex-1);
